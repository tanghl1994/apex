...
//        using namespace at;
        ...
        if (sm_count == -1){
            ...
            checkCudaErrors(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, lamb_cuda_kernel<float,float>, threadsPerBlock, (size_t)2 * threadsPerBlock * sizeof(float) ));
        }
