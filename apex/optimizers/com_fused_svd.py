        ...
        for group, grads_this_group, output_params_this_group, grad_norm_group in zip(self.param_groups, grads_group,
            ...
            for p, grad, output_param, grad_norm in zip(group['params'], grads_this_group, output_params_this_group,
                ...
                if not adam_freeze:
                    ...
                    torch.matmul(matrix, comQ, out=comP)
                    dist.all_reduce(comP)
                    comP /= dist.get_world_size()
                    comP = self._orthogonalize(comP)
                    torch.matmul(matrix.t(), comP, out=comQ)
                    dist.all_reduce(comQ)
                    comQ /= dist.get_world_size()

                    temp_exp = (naive_compress(buffer_exp + ecbuffer, chunk_size=8))
                    # if dist.get_rank() == 0 or dist.get_rank()==16:
                    #   if (state['step']+1)%10 ==0:
