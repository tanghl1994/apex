...
class ECFusedMixed(torch.optim.Optimizer):
    ...
    def __init__(self, params,
        ...
        global fused_sgd_cuda
        fused_sgd_cuda =  importlib.import_module("fused_sgd_cuda")
        ...
        super(ECFusedMixed, self).__init__(params, defaults)
        ...
        for group, grads_this_group, output_params_this_group, grad_norm_group in zip(self.param_groups, grads_group, output_params_group, grad_norms):
            ...
            for p, grad, output_param, grad_norm in zip(group['params'], grads_this_group, output_params_this_group, grad_norm_group):
                ...
                if not adam_freeze:
                    ...
                    scaled_error = EC_buffer * combined_scale
                    com_grad = topk_compress(para.grad.data.float() + scaled_error,alpha = 0.1)
                    
 
                    fused_sgd_cuda.sgd(p.data,
                                       out_p,
                                       exp_avg,
                                       exp_avg_sq,
                                       grad,
