...
class ComFusedTest(torch.optim.Optimizer):
    ...
    def __init__(self, params,
        ...
        super(ComFusedTest, self).__init__(params, defaults)
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=1., grad_norms=None, adam_freeze=False):
        ...
        for group, grads_this_group, output_params_this_group, grad_norm_group in zip(self.param_groups, grads_group, output_params_group, grad_norms):
            ...
            for p, grad, output_param, grad_norm in zip(group['params'], grads_this_group, output_params_this_group, grad_norm_group):
                ...
                if not adam_freeze:
                    ...
                    com_grad = naive_compress(ecbuffer + grad.float()/combined_scale)
                    temp_error = ecbuffer + grad.float()/combined_scale - com_grad
                    ...
                    if self._compute_grad_norm(com_grad) == -1:
                        ...
                        return True
                    if group['max_grad_norm'] > 0:
                        com_norm = torch.norm(com_grad)
                        clip = (com_norm + 1e-6) / ( group['max_grad_norm'])
                        if clip > 1:
                            temp_exp = beta1 * exp_avg + (1 - beta1)*com_grad/clip
                        else:
                            temp_exp = beta1 * exp_avg + (1 - beta1)*com_grad
