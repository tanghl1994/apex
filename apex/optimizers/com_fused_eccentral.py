...
class ComFusedECCentral(torch.optim.Optimizer):
    ...
    def __init__(self, params,
        ...
        w_size() = dist.get_size()
        self.send_groups = [ [ [] for j in range(w_size) ] for i in range(w_size)]
        for i in range(dist.get_world_size() - 1):
            for j in range(dist.get_world_size() - 1 - i):
                group = dist.new_group(ranks = [i][j])
                send_groups[i][j] = group
                send_groups[j][i] = group

        wait_key_list = [ [0 for i in range(w_size)] for j in range(w_size - 1)]
        for t in range(w_size - 1):
            for pos in range(w_size):
                rec = (pos + t + 1)%w_size
                if wait_key_list[t][rec] == 0:
                    wait_key_list[t][pos] == 1
