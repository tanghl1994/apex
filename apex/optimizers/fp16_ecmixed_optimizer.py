...
class FP16_ECMixed_Optimizer(object):
    """
    ...
    def __init__(self,
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            #print("Before :",(self.fp16_groups[i] is param_group['params']))
           ...
           # print("After :",(self.fp16_groups[i] is param_group['params']))
            self.ecbuffer_groups_flat.append(torch.zeros_like(self.fp32_groups_flat[i]).detach())
    ...
    def step(self, closure=None, adam_freeze=False):
        """
          ...
          #  for p in group:
           #     if torch.isnan(p.grad).any():
            ...
            grads_groups_flat.append(_flatten_dense_tensors([p.grad for p in group]))
               ...
               # myskip += 1
               # dist.broadcast(myskip,dist.get_rank())
               # skip = bool(myskip)
            if adam_freeze is True:
                scaled_error = self.ecbuffer_groups_flat[i]*self.cur_scale
                com_grad = naive_compress(grads_groups_flat[i].float() + scaled_error)
                temp_error = (grads_groups_flat[i].float() + scaled_error - com_grad)/self.cur_scale
                if self._compute_grad_norm(temp_error) == -1:
                    print("TempError Exloding")
                    skip = True
                dist.all_reduce(com_grad)
                com_grad/=dist.get_world_size()
                if self._compute_grad_norm(com_grad) == -1:
                    skip = True
                    print("Gradient Exploding")
                else:
                    (grads_groups_flat[i]).set_(com_grad.half())                      
                    self.ecbuffer_groups_flat[i] = temp_error
                    if self._compute_grad_norm(self.ecbuffer_groups_flat[i]) == -1:
                        print("ErrorBuffer Exploding")
            if adam_freeze is False:
                dist.all_reduce(grads_groups_flat[i])
                grads_groups_flat[i]/=dist.get_world_size()
                if self._compute_grad_norm(grads_groups_flat[i]) == -1:
                    skip = True
        ...
        self.optimizer.step(grads=[[g] for g in grads_groups_flat],
                            ...
                            grad_norms=norm_groups,adam_freeze=adam_freeze)
