'''FP16 Optimizer without weight fusion to support LAMB optimizer
    Authors: Samyam Rajbhandari (modified from original FP16 Optimizer in Apex'''

import torch
from torch._utils import _flatten_dense_tensors, _unflatten_dense_tensors

class FP16_Unfused_ECMomentum_Optimizer(object):
    """
    ...
    def step(self, closure=None, adam_freeze = False):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            norm_groups.append([self._compute_grad_norm(p.grad) for p in group])
            
             

        # norm is in fact norm*cur_scale
        skip = self.optimizer.step(grads=grads_groups,
