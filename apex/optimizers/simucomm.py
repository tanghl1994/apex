...
def simusend(origin_tensor, origin_error, buffer_error, send_groups):
    ...
    for t in range(w_size - 1):           
        ...
        pos = (pos + 1)%w_size
        tensor_list[pos] = scale_list[pos] * (2 * sign_list[pos].float() - 1) + origin_list[pos]
        '''if dist.get_rank() == 1 and t==0:
    ...
    scale_list[pos],sign_list[pos],diff = imple_naive_compress(tensor_list[pos] + ori_error_list[pos])
    error_list[pos].set_(diff)
    tensor_list[pos] = scale_list[pos] * (2 * sign_list[pos].float() - 1)
...
def sparse_simusend(origin_tensor, origin_error, buffer_error, send_groups, chunk_size, idx):
    ...
    for t in range(w_size - 1):           
        sign_list[pos],diff = imple_random_sparse_compress(tensor_list[pos] + ori_error_list[pos], alpha = 0.8)
        error_list[pos].set_(diff)
...
def mygather(origin_tensor, ecbuffer, buffer_error, final_ecbuffer, final_buffer_error, groups_list, wait_key_list):
    ...
    rec_tensor = torch.zeros_like(torch.chunk(origin_tensor, w_size)[dist.get_rank()])
    rec_scale = torch.zeros_like(origin_tensor[0])
    rec_sign = torch.zeros_like(rec_tensor)

    ori_error_list = torch.chunk(ecbuffer, w_size)
    error_list = torch.chunk(buffer_error, w_size)

    tensor_list = torch.chunk(origin_tensor, w_size)
    
    
    for t in range(w_size - 1):
        send_pos = send_scheduler_ring(rank, t)
        rec_src = rec_scheduler_ring(rank, t)
        wait_flag = wait_key_list[rank][t]
